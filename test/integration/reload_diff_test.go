...
func TestReloadDiff_NoChange_PointersIdentical(t *testing.T) {
	...
	state1, _, err := server.BuildState(cfg, aliases, nil, nil, nil, nil, server.VerifyModeHash, country, asn, logger)
	if err != nil {
	...
	state2, summary, err := server.BuildState(cfg, aliases, nil, nil, nil, prev, server.VerifyModeHash, country, asn, logger)
	if err != nil {
...
func TestReloadDiff_SameSizeContentChange_HashDetects(t *testing.T) {
	...
	state1Hash, _, err := server.BuildState(cfg, aliases, nil, nil, nil, nil, server.VerifyModeHash, country, asn, logger)
	if err != nil {
		t.Fatalf("initial BuildState (hash): %v", err)
	}

	// --- Build state1 under VerifyModeSize (separate server for size-mode test) ---
	state1Size, _, err := server.BuildState(cfg, aliases, nil, nil, nil, nil, server.VerifyModeSize, country, asn, logger)
	if err != nil {
	...
	state2Hash, summaryHash, err := server.BuildState(cfg, aliases, nil, nil, nil, &state1Hash, server.VerifyModeHash, country, asn, logger)
	if err != nil {
	...
	state2Size, summarySize, err := server.BuildState(cfg, aliases, nil, nil, nil, &state1Size, server.VerifyModeSize, country, asn, logger)
	if err != nil {
...
func TestReloadDiff_NoneMode_AlwaysReparses(t *testing.T) {
	...
	state1, _, err := server.BuildState(cfg, aliases, nil, nil, nil, nil, server.VerifyModeNone, country, asn, logger)
	if err != nil {
	...
	state2, summary, err := server.BuildState(cfg, aliases, nil, nil, nil, &state1, server.VerifyModeNone, country, asn, logger)
	if err != nil {
