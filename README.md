...
- DNS-over-HTTPS (DoH, RFC 8484) — opt-in via the `doh` section in shadowdns.yaml. Serves `/dns-query` (GET base64url + POST `application/dns-message`) over HTTPS by reusing the authoritative query path, so DoH answers are identical to UDP/TCP. Authoritative and non-recursive: only ShadowDNS-hosted zones are answered, out-of-zone queries get REFUSED. The TLS certificate is issued for the configured IP via ACME HTTP-01 using the Let's Encrypt short-lived profile and auto-renewed/hot-swapped without restart. DoH queries are labeled `proto="doh"` in metrics. See [docs/guides/doh.md](docs/guides/doh.md)
