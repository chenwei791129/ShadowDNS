...
* **dns:** preserve DNS name case across the response path per RFC 4343. The Question section now echoes the on-wire qname byte-for-byte (required for DNS-0x20 case-randomized queries to authenticate). Answer / Authority / Additional owner names use zone-file storage case for exact matches, and qname case for wildcard synthesis. Alias rewrites preserve the qname prefix and use the operator-authored YAML case for the backup suffix. Operators should write backup names in `shadowdns.yaml` with the case they want clients to observe; matching remains case-insensitive (RFC 1035 §2.3.3) so existing all-lowercase configs are unaffected.
