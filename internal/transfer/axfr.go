...
func buildAliasRecords(rootZone, backupZone *zone.Zone, rootOrigin, backupOrigin string) []dns.RR {
	...
	if backupZone != nil {
		for ownerFqdn, rrs := range backupZone.Records {
			for _, rr := range rrs {
				k := overrideKey(ownerFqdn, rr.Header().Rrtype)
				overrides[k] = append(overrides[k], rr)
			}
		}
	}
