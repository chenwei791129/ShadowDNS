...
1. Copy a production `named.conf`, `master.zones`, and the zone file directory to the test environment.
...
How to handle it: remove the directive from `named.conf` or `master.zones`, or exclude the corresponding zone from ShadowDNS's configuration scope.
...
|----------|----------|
| Zone data (zone files, `master.zones`) | SIGHUP reload |
