...
狀況，作為 ShadowDNS 是否將此功能從 [README.md](../README.md) 中的 *Planned* 推進為實作功能
的決策依據。
...
> **與 ECS 調查的關鍵差異**：ECS 是 resolver ↔ authoritative 雙邊協作功能；CNAME Flattening
> 則是**純 authoritative 端**功能——由 authoritative server 自己把 apex 上的「類 CNAME」記錄
> 解析成 A/AAAA 後直接回覆。因此本報告沒有對應 ECS 調查的「遞迴解析器」一節，改以「標準化
> 背景」一節取代。
...
但與 ECS 不同，這次的版圖呈現明顯的**「開源弱、商業強」分裂**：
...
- **三大傳統 server（BIND / NSD / Knot）的一致拒絕，對 ShadowDNS 是個定位參照**：不做這功能
  並不會讓 ShadowDNS 在「嚴肅 authoritative server」這個分類中顯得落後——這與 ECS 的情況
  （gdnsd / PowerDNS / Knot 都支援、不做才落後）恰恰相反。
...
### 該不該做？傾向「限定 in-bailiwick 目標即可做，但優先級仍低於 ECS」

**支持實作的理由：**

1. **DNSSEC 衝突對 ShadowDNS 不存在**：CNAME Flattening 最大的技術障礙是無法與 DNSSEC 共存
   ——但 ShadowDNS [README.md](../README.md) 明確不支援 DNSSEC，**這條最硬的約束直接免除**。
...
| 維度 | ECS（Planned） | CNAME Flattening（Planned） |
|---|---|---|
| 開源同類是否都有 | gdnsd/PowerDNS/Knot 有，**不做會落後** | 只有 PowerDNS/Technitium 有，BIND/NSD/Knot 都沒有，**不做不落後** |
| 與核心架構衝突 | 無（純讀取 query 內容） | 通用版**高**；限 in-bailiwick 後**消除**（純記憶體查詢） |
| DNSSEC 障礙 | 無 | 無（ShadowDNS 本就不支援，免除） |
| GeoIP 互動 | **強化** geo 精度 | 通用版**削弱**；限 in-bailiwick 後**保留**（同 view 解析） |
| IETF 標準狀態 | RFC 7871（Informational，停滯） | 無 RFC（ANAME 已死，轉向 RFC 9460 SVCB/HTTPS） |
...
> 代價是只服務「apex 指向本地 zone」、不服務「apex 指向外部 CDN」。建議優先級**仍低於
> ECS**；若需求其實是外部 CDN，則應改評估 RFC 9460 HTTPS record。
