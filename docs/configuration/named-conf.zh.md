...
- **未定義的參照採 fail-closed**：正向參照到沒有 `acl` 定義的名稱時會被丟棄並記 WARN 且永不命中 —— 該 view 不服務任何 client，而非匹配所有人。**否定**的未定義參照（`!name`）則以「拒絕整個清單」取代而非丟棄，使它所表達的排除語意不會悄悄放寬存取。
