...
#### Scenario: backup without a matching root in the same view aborts that pair

- **WHEN** a view declares a backup zone but does not declare the aliased root zone in the same view
- **THEN** the sub-command SHALL emit a warning identifying the `(view, backup)` pair AND SHALL skip that pair AND SHALL NOT abort processing of other pairs
