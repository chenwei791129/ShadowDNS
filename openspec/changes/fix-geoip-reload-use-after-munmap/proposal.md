...
- Keep an explicit close of the *current* generation at process shutdown only (after the reload goroutine is joined, when no query is in flight). The dependency's close is idempotent, so this shutdown close cannot double-unmap even if the cleanup later runs.
...
- Process shutdown still closes the current generation exactly once and is safe to call twice (the GeoIP-runtime close-all remains idempotent at the holder level) — verified by the existing close-all-is-idempotent test, updated for the removed parked-generation slots.
